...
(function () {
  ...
  /* Hidden background music — official YouTube embed. */
  ...
  function updateMusicToggle() {
    ...
    musicToggle.title = musicMuted ? 'Play background music' : 'Mute background music';
    musicToggle.querySelector('span').textContent = musicMuted ? '♩' : '♪';
  }
  ...
  function startBackgroundMusic() {
    ...
    } catch { /* Player is still initializing. */ }
  }

  window.onYouTubeIframeAPIReady = function () {
    musicPlayer = new YT.Player('background-music-player', {
      width: '1', height: '1', videoId: '2up_Eq6r6Ko',
      playerVars: {
        autoplay: 1, controls: 0, disablekb: 1, fs: 0, loop: 1,
        modestbranding: 1, playlist: '2up_Eq6r6Ko', playsinline: 1,
  ...
  function loadYouTubeApi() {
    if (youtubeApiLoading || musicReady) return;
  ...
  window.addEventListener('load', function () {
    if ('requestIdleCallback' in window) {
      window.requestIdleCallback(loadYouTubeApi, { timeout: 1200 });
    } else {
  ...
  document.addEventListener('pointerdown', requestBackgroundMusic, { once: true, passive: true });
  document.addEventListener('keydown', requestBackgroundMusic, { once: true });
  ...
  musicToggle.addEventListener('click', function (event) {
    ...
    if (musicMuted) musicPlayer.pauseVideo();
